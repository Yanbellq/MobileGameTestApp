...
>(({ children, className, ...props }, ref) => {
  return (
    <View className="flex w-full flex-col items-center overflow-hidden rounded text-muted-foreground">
      <GlassView
        glassEffectStyle="clear"
        // intensity={40}
        // Додаємо базові стилі скляної панелі, схожі на Input
        className={cn(
          'flex w-full flex-col items-center overflow-hidden bg-primary',
          tableStyle({ class: className })
        )}>
        <ExpoTable ref={ref} {...props} className="w-full">
          {children}
        </ExpoTable>
      </GlassView>
    </View>
